...
const services = [
  { icon: Search, title: "SEO & Growth", desc: "On-page, technical and off-page SEO that compounds." },
  { icon: Megaphone, title: "Performance Marketing", desc: "Paid social and search that pays for itself." },
  { icon: Palette, title: "Branding & Design", desc: "Identities, creatives and ads with a point of view." },
  { icon: Sparkles, title: "Content Strategy", desc: "Story-led content that turns scrollers into clients." },
];
...
const episodes = [
  { title: "Mindset shifts for ambitious creators", duration: "32 min", cat: "Mindset" },
  { title: "Building a brand from scratch in 2025", duration: "41 min", cat: "Career" },
  { title: "Real talk: burnout and boundaries", duration: "28 min", cat: "Real Talk" },
];

const testimonials = [
  {
    quote: "Pallabi rebuilt our brand and our growth engine in 60 days. Revenue is up 3.4x and we finally feel premium.",
    name: "Aarav Mehta",
    role: "Founder, Kindred Studio",
  },
  {
    quote: "The clearest marketing thinker I've worked with. Strategy + execution under one roof. Rare.",
    name: "Ishita Roy",
    role: "Director, Lumen Labs",
  },
  {
    quote: "Our SEO traffic 6x'd in five months. She just gets the modern internet.",
    name: "David Cole",
    role: "CEO, Northbeam",
  },
];
...
function HomePage() {
  ...
  useEffect(() => {
    getCourses().then(({ data }) => setCourses((data ?? []).slice(0, 3)));
  ...
  return (
    <>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-12 items-center">
          <div className="md:col-span-1 lg:col-span-7 space-y-8 animate-fade-up">
            ...
            <p className="text-lg text-muted-foreground max-w-xl leading-relaxed">
              ...
              storytelling and design to help ambitious founders win the modern internet.
            </p>
            <div className="flex flex-wrap gap-3">
              <GradientButton to="/contact">Work with me</GradientButton>
              <GradientButton to="/blog" variant="ghost">Explore my work</GradientButton>
            </div>
        ...
        <div className="flex animate-marquee whitespace-nowrap gap-12 text-2xl font-display text-muted-foreground/60">
          {Array.from({ length: 2 }).map((_, k) => (
            <div key={k} className="flex gap-12">
              {["Strategy","✦","SEO","✦","Branding","✦","Performance","✦","Content","✦","Design","✦","Storytelling","✦"].map((w, i) => (
                <span key={i}>{w}</span>
              ))}
            </div>
          ))}
      ...
      <Section>
        ...
        <div className="mt-10">
          <Link to="/about" className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:text-primary-glow transition-colors">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mt-14">
          {services.map((s) => (
            <div key={s.title} className="glass-card p-6 group">
              ...
              <Link to="/services" className="mt-5 inline-flex items-center gap-1.5 text-xs font-semibold text-primary group-hover:gap-3 transition-all">
      ...
      <Section>
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          />
          <Link to="/blog" className="text-sm font-semibold text-primary inline-flex items-center gap-2">
        ...
        <div className="grid md:grid-cols-3 gap-5 mt-14">
          {works.map((w, i) => (
            <Link
              to="/blog"
              key={w.title}
              className={`group relative overflow-hidden rounded-3xl border border-border/40 ${i === 0 ? "md:row-span-2 md:col-span-1" : ""}`}
            >
              <img src={w.img} alt={w.title} loading="lazy" className="w-full h-72 md:h-full object-cover transition-transform duration-700 group-hover:scale-110" />
      ...
      <Section>
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="relative rounded-3xl overflow-hidden gradient-border p-1">
            <img src={podcastCover} alt="Pallabi Talks podcast cover" loading="lazy" className="w-full rounded-[calc(var(--radius-xl)-2px)]" />
          </div>
          <div>
            <SectionHeading
              ...
              {episodes.map((e, i) => (
                <li key={e.title} className="glass-card p-5 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="text-xs text-primary uppercase tracking-widest">Ep · 0{i + 1} · {e.cat}</div>
            ...
            {books.map((book) => (
              ...
              >
                ...
                <div className="mt-10 space-y-3 px-2">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed max-w-[260px] mx-auto">{book.description}</p>
                  <div className="pt-4">
          ...
          <div>
            ...
            <div className="mt-8 glass-card p-7 space-y-5">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="text-xs uppercase tracking-widest text-primary font-semibold mb-2">Published · Brainware University</div>
      ...
      {courses.length > 0 && (
        <Section>
          ...
          <div className="grid md:grid-cols-3 gap-5 mt-14">
            {courses.map((c) => (
              <div key={c.id} className="glass-card p-7">
                ...
                <p className="mt-1 text-sm text-muted-foreground">{c.duration} · {c.tag}</p>
                ...
                <div className="mt-6 flex items-center justify-between">
                  ...
                  <Link to="/courses" className="rounded-full glass px-4 py-2 text-xs font-semibold hover:border-primary/40">
                    Enroll
                  </Link>
                </div>
              </div>
        ...
        <div className="relative overflow-hidden rounded-[2rem] glass-card p-10 sm:p-16 text-center noise">
          ...
          <div className="relative">
            ...
            <p className="mt-6 max-w-xl mx-auto text-muted-foreground">
              Book a free 20-minute strategy call. Walk away with a clear next move — whether we work together or not.
            </p>
